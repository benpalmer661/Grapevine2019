...
//  Grapevine2019
//
//  Created by Ben Palmer on 26/3/18.
...
class BlockedUsers: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    fileprivate func firestoreFetchContactsUserIds() {
        ...
        db.collection("Users").document(uid).collection("Users Blocked By Me").getDocuments() { (querySnapshot, err) in
            if let err = err {
